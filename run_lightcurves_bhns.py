...
def myprior_targeted(cube, ndim, nparams):
        ...
        cube[1] = cube[1]*0.1 + 3.0
        cube[2] = cube[2]*0.01 + 0.3
        cube[3] = cube[3]*0.1 + 0.1
        cube[4] = cube[4]*0.04 + 1.46
...
pymultinest.run(myloglike, myprior, n_params, importance_nested_sampling = False, resume = True, verbose = True, sampling_efficiency = 'parameter', n_live_points = 100, outputfiles_basename='%s/2-'%plotDir, evidence_tolerance = 0.5, multimodal = False)

#pymultinest.run(myloglike, myprior_targeted, n_params, importance_nested_sampling = False, resume = True, verbose = True, sampling_efficiency = 'parameter', n_live_points = 100, outputfiles_basename='%s/2-'%plotDir, evidence_tolerance = 0.5, multimodal = False)
