    ...
    if doAB:
        if not Global.svd_mag_model == 0:
            ...
            if LoadModel:
                ...
                svd_mag_model = svd_utils.calc_svd_mag(table['tini'][0], table['tmax'][0], table['dt'][0], model = "Bu2019rp", n_coeff = table['n_coeff'][0])
                modelfile = os.path.join(ModelPath,'Bu2019rp_mag.pkl')
        ...
        if not Global.svd_lbol_model == 0:
            ...
            if LoadModel:
                ...
                svd_lbol_model = svd_utils.calc_svd_lbol(table['tini'][0], table['tmax'][0], table['dt'][0], model = "Bu2019rp", n_coeff = table['n_coeff'][0])
                modelfile = os.path.join(ModelPath,'Bu2019rp_lbol.pkl')
                with open(modelfile, 'wb') as handle:
