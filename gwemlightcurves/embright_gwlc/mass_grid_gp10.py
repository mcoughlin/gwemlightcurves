...
def run_EOS(EOS, m1, m2, thetas, type_set = 'None', N_EOS = 100, model_set = 'Bu2019inc', chirp_q = False):
    ...
    if EOS == "gp":
        ...
        eospostdat = np.genfromtxt("~/em-bright/ligo/em_bright/EOS_samples_unit_test/eos_post_PSRs+GW170817+J0030.csv",names=True,dtype=None,delimiter=",")
    ...
    for ii, row in enumerate(samples): 
        ...
        for jj in range(nsamples):
            ...
            if EOS == "spec":
                ...
                while (lambda1 < 0.) or (lambda2 < 0.) or (mbns < 0.):
                    phasetr = 0
                    eospath = "~/em-bright/ligo/em_bright/EOS_samples_unit_test/MACROdraw-%06d-%d.csv" % (idxs[index], phasetr)
